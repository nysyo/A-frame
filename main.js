...
function render() {
    if(t < 2*Math.PI){
        ...
        y = defalt.y;
        z = (1-b)*Math.sin(t)+a*Math.sin(t-t/b)+defalt.z;
        var line = document.createElement('a-entity');
        line.setAttribute("line", `start: ${prePos}; end: ${x} ${y} ${z}; color: #136FFF`);
        requestAnimationFrame(render);
        scene.append(line);
        move_circle((1-b)*Math.cos(t)+defalt.x,defalt.y,(1-b)*Math.sin(t)+defalt.z,b);
        bar.setAttribute("line", `start:${(1-b)*Math.cos(t)+defalt.x} ${defalt.y} ${(1-b)*Math.sin(t)+defalt.z}; end: ${(1-b)*Math.cos(t)+a*Math.cos(t-t/b)+defalt.x} ${defalt.y} ${(1-b)*Math.sin(t)+a*Math.sin(t-t/b)+defalt.z};`);     
        prePos = `${x} ${y} ${z}`;
    }
}
render();